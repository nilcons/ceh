...
+ We use i686 Nix for package management and if you can decide the
  ...
  work on any modern GNU/Linux, we also tested Ubuntu trusty and precise
  both i686 and amd64.
+ When it makes sense, we provide 64-bit version of packages too,
  e.g. you can use 64-bit haskell by setting the =CEH_GHC64=
...
can save a lot of memory and disk by choosing 32-bit.  Memory at VPS
providers can be especially expensive.
...
(e.g. project euler).  Or when you just want to test your Haskell
program on both architecture.

For this reason, as an exception for GHC, we support turning on 64-bit
...
searched for in =/opt/ceh/installed/ghclibs(64)=.
...
# BerkeleyDB for CEH_GHC64=1
ceh_nixpkgs_install_ghclibs('db', bit64 => 1, AUTOINIT);
#+END_SRC
...
+ Update =ENSURE_BASE_PERL=, =ENSURE_BASE_NIXPATH= and
  =CEH_NIX_DOWNLOAD= in =scripts/ceh-init.sh=.
