[[https://travis-ci.org/errge/ceh][https://travis-ci.org/errge/ceh.png]]
...
After this operation you have to re-run =ceh-init.sh= to re-create the
=/nix/var/nix/profiles/ceh/essential= profile.

=ceh-init.sh= is idempotent, feel free to run it if you think it can
...
: CEH_AUTO_UPGRADE=1 \
: ENSURE_BASE_PERL=/nix/var/nix/profiles/ceh/essential/bin/perl \
: ENSURE_BASE_NIXPATH=/nix/var/nix/profiles/ceh/essential/ /opt/ceh/lib/ensure_base_installed.pl

+ Update CEH_BASELINE_PERL in =lib/perl= (peek from CehInstall.pl:ensure_base_installed).
